...
namespace CPP_Metrics
{
    ...
    public class UsedVariables : CPP14ParserBaseVisitor<bool>
    {
        ...
        public override bool VisitPostfixExpression([NotNull] CPP14Parser.PostfixExpressionContext context)
        {
            ...
            if(parentParent is CPP14Parser.PostfixExpressionContext postfixParentParent)
            {
                if (postfixParentParent.LeftParen() is not null || postfixParentParent.RightParen() is not null) return true;
            }
            ...
            var Identifier = context.primaryExpression()?.idExpression()?.unqualifiedId()?.Identifier();
